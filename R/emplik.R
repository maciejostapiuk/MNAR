...
#'
#' @title empirical likelihood method based on momentfit package, supported by Morikawa and Kim (2018)
#'
...
emplik <- function(data, X, totals, dweights, propensity, g_function, initial_phi) {
  # Define the calibration equation
  calibration_eq <- function(phi) {
    g_values <- g_function(X)
    weights <- dweights / propensity(phi, data)
    colSums(weights * g_values) - totals
  }

  # Solve the calibration equation
  result <- nleqslv(initial_phi, calibration_eq)
  phi_hat <- result$x

  # Calculate and return the corrected weights
  corrected_weights <- dweights / propensity(phi_hat, data)
